...
class BlogController extends AbstractController
{
    ...
    private function getResponseStartHTML(): string
    {
        return <<<HTML
<!DOCTYPE html>
<html>
...
HTML;
    ...
    public function mainPage(): Response
    {
        ...
        $response = $this->responseStart . '<h1>Articles</h1><ul>';

        foreach ($articles as $index => $article) {
            $response .= $this->generateArticleListItem($article, $index);
        }

        $response .= $this->generateActionButtons() . '</ul></div></body></html>';
        return new Response($response);
    }

    private function generateArticleListItem(Articles $article, int $index): string
    {
        return sprintf(
    ...
    private function generateActionButtons(): string
    {
        return <<<HTML
<li>
    <button onclick="window.location.href='/article/add'" style="margin: 16px auto;">Add Article</button>
    <button onclick="window.location.href='/author/add'" style="margin: 16px auto;">Add Author</button>
</li>
HTML;
    }
    ...
    public function showArticle(int $id): Response
    {
        ...
        $response = $this->responseStart . '<h1 style="margin-bottom: 5px">' . htmlspecialchars($article->getTitle()) . '</h1>';
        $response .= '<h5 style="margin: 0">by ' . $article->getAuthor()->getName() . '</h5><br>' . $article->getContent() . '<br><button onclick="window.location.href=\'/articles\'" style="margin: 12px auto">Go Back</button>';
    ...
    public function addArticleForm(): Response
    {
        ...
        return new Response(file_get_contents($htmlFilePath), 200, ['Content-Type' => 'text/html']);
    }
